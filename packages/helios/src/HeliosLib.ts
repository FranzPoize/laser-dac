...
const HeliosLib = {
    ...
    //returns 1 if successful
    WriteFrame: helios.func(
        'int WriteFrame(uint, uint, uint8_t, HeliosPoint *, uint)',
    ),
    GetMaxSampleRate: helios.func(
        'unsigned int GetMaxSampleRate(unsigned int devNum)',
    ),
    GetMinSampleRate: helios.func(
        'unsigned int GetMinSampleRate(unsigned int devNum)',
    ),
    GetMaxFrameSize: helios.func(
        'unsigned int GetMaxFrameSize(unsigned int devNum)',
    ),
    ReScanDevices: helios.func('int ReScanDevices()'),
};
...
export function rescanDevices(): number {
    return HeliosLib.ReScanDevices();
}
